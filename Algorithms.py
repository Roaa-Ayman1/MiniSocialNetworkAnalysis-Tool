...
def partition_graph(G, attribute):
    ...
    for node, data in G.nodes(data=True):
        ...
        print("hola")
    return visualize_partitioned_graphs(partitions,G)
# def visualize_partitioned_graphs(partitions, G):
#     print("innnnn")
...
def visualize_partitioned_graphs(partitions, G):
    ...
    """
    try:
        print("Drawing partitions...")  # Debug print
        ...
        for i, (label, nodes) in enumerate(partitions.items()):
            ...
            display_label = str(label) if label is not None else f"Partition {i+1}"
            
            color_index = i % len(colors)
            plt.subplot(1, num_partitions, i+1)
            
            # Create subgraph safely
            subgraph = G.subgraph(nodes) if nodes else nx.Graph()
            
            # Draw with fallback for empty subgraphs
            if subgraph and len(subgraph.nodes()) > 0:
                nx.draw(subgraph, 
                       ...
                       node_size=300,
                       width=1.0)
                plt.title(f"Partition: {display_label}\n"
                         f"Nodes: {len(subgraph.nodes())} "
                         f"Edges: {len(subgraph.edges())}")
            else:
                plt.text(0.5, 0.5, 
                        f"Empty Partition: {display_label}",
                        ha='center', va='center')
                plt.title(f"Empty: {display_label}")
                
        plt.tight_layout()
        plt.show()
        
    except Exception as e:
        print(f"Error drawing partitions: {str(e)}")
        # Optionally show error in GUI
        import tkinter.messagebox as msgbox
        msgbox.showerror("Visualization Error", f"Could not draw partitions: {str(e)}")
####################################################################
...
def Statistics_Of_Graph_Metrics(G):
...
def Girvan_Newman_algorithm_single_level(G):
    G_copy = G.copy()
    ...
    return num_communities, modularity
####################################################################
def Comparing_Community_Detection(G):
...
def adjust_graph(G, 
                node_color='pink', edge_color='gray', 
                node_shape='o', base_node_size=300,
                label_attribute=None, 
                node_color_attribute=None, node_size_attribute=None,
                base_edge_width=1.0, edge_width_attribute=None,
                gender_filter=None,
                layout_type='spring'):
    """
    ...
    try:
        ...
        if gender_filter is not None:
            try:
                filtered_nodes = [
                    ...
                    if str(H.nodes[node].get('gender', '')).lower() == str(gender_filter).lower()
                ]
                if filtered_nodes:
                    H = H.subgraph(filtered_nodes)
                else:
                    print(f"No nodes with gender '{gender_filter}' found - showing full graph")
            except Exception as e:
                print(f"Gender filter error: {str(e)} - showing full graph")

        # Process node colors
        if node_color_attribute:
            try:
                node_colors = [
                    H.nodes[node].get(node_color_attribute, node_color)
                    for node in H.nodes()
                ]
                # Try converting to numeric for colormap
                try:
                    node_colors = [float(c) for c in node_colors]
                except (ValueError, TypeError):
                    pass  # Use as categorical colors
            except Exception:
                node_colors = node_color
                print(f"Using default color due to invalid color attribute")
        else:
            node_colors = node_color

        # Process node sizes with robust validation
        node_sizes = []
        if node_size_attribute:
            try:
                size_values = []
                for node in H.nodes():
                    val = H.nodes[node].get(node_size_attribute)
                    try:
                        size_values.append(float(val) if val is not None else 1.0)
                    except (ValueError, TypeError):
                        size_values.append(1.0)
                
                if size_values:
                    max_size = max(size_values) or 1.0
                    node_sizes = [base_node_size * (val/max_size) for val in size_values]
                else:
                    node_sizes = base_node_size
            except Exception as e:
                print(f"Size attribute error: {str(e)} - using default size")
                node_sizes = base_node_size
        else:
            node_sizes = base_node_size

        # Ensure node_sizes is properly formatted
        if isinstance(node_sizes, (int, float)):
            node_sizes = [node_sizes] * len(H.nodes())
        elif len(node_sizes) != len(H.nodes()):
            node_sizes = [base_node_size] * len(H.nodes())

        # Process edge widths
        edge_widths = base_edge_width
        if edge_width_attribute and H.edges():
            try:
                width_values = [
                    float(H.edges[edge].get(edge_width_attribute, base_edge_width))
                    for edge in H.edges()
                ]
                max_width = max(width_values) or 1.0
                edge_widths = [base_edge_width * (val/max_width) for val in width_values]
            except Exception:
                edge_widths = base_edge_width
                print("Using default edge width")
        ...
        if label_attribute:
            try:
                labels = {
                    node: str(H.nodes[node].get(label_attribute, node))
                    for node in H.nodes()
                }
            except Exception:
                labels = {node: str(node) for node in H.nodes()}
                print("Using node names as labels")

        # Create visualization
        plt.figure(figsize=(12, 8))
        
        pos = nx.spring_layout(H) if layout_type == 'spring' else nx.circular_layout(H)
        ...
        plt.title(f"Network: {len(H.nodes())} nodes, {len(H.edges())} edges")
        plt.show()

    except Exception as e:
        print(f"Visualization failed: {str(e)}")
        import tkinter.messagebox as msgbox
        msgbox.showerror("Error", f"Graph visualization failed:\n{str(e)}")
...
def draw_partitioned_graph_centrality(G, clusters):
    ...
    # Show the plot
    plt.show()
def partitioning_by_closeness(G):
...
def filter_and_visualize_graph(G, degree_range=(0, float('inf')), node_color='skyblue', 
                             edge_color='gray', node_size=300, edge_width=1.0):
    """
    ...
    try:
        ...
        if not filtered_nodes:
            raise ValueError(f"No nodes found with degree between {min_degree} and {max_degree}")
        ...
        plt.title(f"Filtered Network (Degree {min_degree}-{max_degree})\n"
                 f"{len(H.nodes())} nodes, {len(H.edges())} edges")
        plt.show()
        
    except Exception as e:
        print(f"Filter Error: {str(e)}")
        # Optionally show error message in GUI
        import tkinter.messagebox as msgbox
        msgbox.showerror("Filter Error", str(e))